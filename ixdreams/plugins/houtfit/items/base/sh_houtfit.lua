...
if (CLIENT) then
    function ITEM:PaintOver(item, w, h)
        if (item:GetData("equip")) then
            surface.SetDrawColor(110, 115, 255, 100)
...
ITEM.functions.EquipUn = {
    ...
    OnRun = function(item)
        ...
        item.player:EmitSound("interlock/inventory/inv_pouch_close_01.ogg", 70, math.random(90,110))
...
ITEM.functions.Equip = {
    ...
    OnRun = function(item, creationClient)
        ...
        ply:EmitSound("interlock/inventory/inv_pouch_open_01.ogg", 70, math.random(90,110))
...
function ITEM:Repair(ply, amount)
    ...
    local repairItem = ply:GetCharacter():GetInventory():HasItem("screwdriver")
...
ITEM:Hook("OnInstanced", "InitializeHoutfitArmor", function(invID, x, y, item, data)
    if (item.maxArmor) then
        item:SetData("armor", self.maxArmor)
    end
end)

ITEM:Hook("OnRemoved", "StripDeletedHoutfit", function(item, data)
    if (item.invID != 0 and item:GetData("equip")) then
        item.player = item:GetOwner()
        item:RemoveOutfit(item.player)
