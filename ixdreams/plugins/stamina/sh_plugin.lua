...
local function CalcStaminaChange(client)
	...
	if (SERVER) then
		runSpeed = ix.config.Get("runSpeed") + character:GetAttribute("mob", 0)
	...
	if (client:KeyDown(IN_SPEED) and client:GetVelocity():LengthSqr() >= (walkSpeed * walkSpeed)) then
		...
		offset = -ix.config.Get("staminaDrain", 1) + math.min(character:GetAttribute("ene", 0), maxAttributes) / 100
	else
	...
	if (CLIENT) then
		...
		local current = client:GetLocalVar("mob", 0)
		local value = math.Clamp(current + offset, 0, 100)

		if (current != value) then
			client:SetLocalVar("mob", value)
			...
			if (value == 0 and !client:GetNetVar("brth", false)) then
				...
				local character = client:GetCharacter()
				local scaleFactor = (character:GetNeed("nutrition") and character:GetNeed("nutrition") / 100) or 1

				character:UpdateAttrib("ene", 0.5 * scaleFactor)
				character:UpdateAttrib("mob", 0.05 * scaleFactor)

				hook.Run("PlayerStaminaLost", client)
			elseif (value >= 10 and client:GetNetVar("brth", false)) then
...
if (SERVER) then
	...
	function PLUGIN:CharacterPreSave(character)
		...
		if (IsValid(client)) then
			character:SetData("stamina", client:GetLocalVar("mob", 0))
		end
	end

	function PLUGIN:PlayerLoadedCharacter(client, character)
		timer.Simple(0.25, function()
			client:SetLocalVar("mob", character:GetData("stamina", 100))
		end)
	end
	...
	function playerMeta:RestoreStamina(amount)
		local current = self:GetLocalVar("mob", 0)
		local value = math.Clamp(current + amount, 0, 100)

		self:SetLocalVar("mob", value)
	end
	...
	end, Color(100, 255, 100), 3, "mob", "ixgui/falling.png")
end
