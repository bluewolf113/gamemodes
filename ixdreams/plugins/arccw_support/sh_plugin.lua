	...
	do
		...
		for _, SWEP in ipairs(weapons.GetList()) do
			...
			if (weapons.IsBasedOn(class, "arccw_base")) then
				...
				local bFileExists = ix.item.list[class] ~= nil and ix.item.list[class] ~= {}
				...
				if (item and item.isArcCW and item.isWeapon) then
					...
					item.class = class
					item.description = bFileExists and item.description or SWEP.Trivia_Desc or ""
					item.model = bFileExists and item.model or SWEP.WorldModel and Model(SWEP.WorldModel) or SWEP:GetWeaponWorldmodel() or "models/weapons/w_pistol.mdl"
					item.name = bFileExists and item.name or SWEP.PrintName or SWEP.TrueName
		...
		for attID, v in pairs(ArcCW.AttachmentTable) do
			...
			elseif v.ixCreateItem then
				local bFileExists = ix.item.list[class] ~= nil and ix.item.list[class] ~= {}
				item = ix.item.list[attID] or ix.item.Register(attID, "base_arccw_attachments", nil, nil, true)
				item.name = bFileExists and item.name or v.PrintName or v.ShortName
				item.description = bFileExists and item.description or v.Description
				item.model = bFileExists and item.model or v.Model or "models/Items/BoxMRounds.mdl"
