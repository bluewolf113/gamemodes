...
function PANEL:Init()
	...
	characters:SetText(mainMenuText)
...
function PANEL:TransitionSubpanel(id)
	...
	if (IsValid(lastSubpanel) and istable(lastSubpanel.info)) and lastSubpanel.info.PreOnDeselected then
		lastSubpanel.info:PreOnDeselected(lastSubpanel)
	end
...
function PANEL:Remove()
	...
	if (IsValid(ix.gui.inv1) and ix.gui.inv1.childPanels) then
		...
		for _, tbl in pairs(ix.gui.inv1.panels) do
			if tbl.Icon then
				tbl.Icon:Remove()
			end
		end
		...
		for i = 1, #ix.gui.inv1.childPanels do
			...
			if (IsValid(panel)) then		
				...
				for _, tbl in pairs(panel.panels) do
					if tbl.Icon then
						tbl.Icon:Remove()
					end
				end
