...
local buttonFadeColor = Color(200, 200, 200, 255)
...
function PANEL:Init()
	self:SetFont("HudSelectionText")
	self:SetTextColor(color_white)
	self:SetPaintBackground(false)
	self:SetContentAlignment(4)
	self:SetTextInset(buttonPadding, 0)

	self.padding = {0, 10, 0, 0} -- left, top, right, bottom
...
function PANEL:PaintBackground(width, height)
	local alpha = 0 -- self.selected and 255 or self.currentBackgroundAlpha
