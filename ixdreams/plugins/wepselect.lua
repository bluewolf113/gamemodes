...
if CLIENT then
    ...
    function PLUGIN:HUDShouldDraw(name)
        return name ~= "CHudWeaponSelection"
    end
    ...
    function PLUGIN:HUDPaint()
        ...
        if fraction <= 0.01 then return end

        local x, y = ScrW() * 0.5, ScrH() * 0.5
        local spacing = math.pi * 0.85
        local radius = 240 * fraction
        ...
        for i = 1, #weapons do
            local theta = (i - self.deltaIndex) * 0.1
    ...
    function PLUGIN:PlayerBindPress(client, bind, pressed)
        ...
        -- Bypass scroll input if Left Alt is held
        if input.IsKeyDown(KEY_LALT) then
            return
        end

        if not (bind:find("invprev") or bind:find("invnext") or bind:find("slot") or bind:find("attack")) then
            return
        end
