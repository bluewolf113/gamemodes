...
ITEM.functions.CPour = {
    ...
    OnRun = function(item)
        ...
        -- Trace downward from just in front of the player
        ...
        if tr.Hit then
            ...
            ix.util.Notify("No surface found to pour onto.", client)
        end
...
ITEM.functions.WaterPlant = {
    name = "Water Plant",
    icon = "icon16/water.png",

    OnRun = function(item)
        local client = item.player
        local volume = item:GetVolume()
        if not volume or volume <= 0 then return false end

        local trace = client:GetEyeTrace()
        local plant = trace.Entity

        if not IsValid(plant) or plant:GetClass() ~= "ix_plant" then
            if SERVER then
                client:Notify("Not a valid plant!")
            end
            return false
        end

        -- Add 1 growth point
        if SERVER and isfunction(plant.SetGrowthPoints) then
            local current = plant:GetGrowthPoints() or 0
            plant:SetGrowthPoints(current + 1)
        end

        -- Consume 250 ml of water
        item:SetVolume(volume - 250)

        -- Play sound
        local liquid = ix.liquids.Get(item:GetLiquid())
        if liquid then
            client:EmitSound("ambient/levels/canals/toxic_slime_sizzle1.wav")
        end
    ...
    OnCanRun = function(item)
        ...
        return IsValid(ent) and ent:GetClass() == "ix_plant" and not ent:GetNetVar("grown", false)
    end
}
