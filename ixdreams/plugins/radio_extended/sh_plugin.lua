...
function PLUGIN:OverwriteClasses()
	...
	do
		...
		function CLASS:OnChatAdd(speaker, text, bAnonymous, data)
			...
			if ((ix.config.Get("enableCallsigns",true)) and (data.callsign) and (data.callsign != "")) then
				...
				name = hook.Run("GetCharacterName", speaker, chatType) or speaker:Name()
			end
			--
	...
	do
		...
		function CLASS:OnChatAdd(speaker, text, bAnonymous, data)
			...
			chat.AddText(self:GetColor(speaker,text), string.format(self.format, hook.Run("GetCharacterName", speaker, chatType) or speaker:Name(), text))
		end
