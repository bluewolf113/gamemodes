...
function ENT:OnVarChanged(var, old, new)
    if SERVER then
        ...
        elseif var == "RadioID" then
            ...
            if radioTable then
                self:SetModel("models/props_interiors/tv.mdl")
            end
        end
    else
        if var == "Enabled" then
...
if SERVER then
    function ENT:Initialize()
        self:SetModel("models/props_interiors/tv.mdl") -- force TV model on spawn
    ...
    function ENT:OnOptionSelected(client, option, data)
        ...
        if option == "Enable" then
            ...
            self:SetSkin(1) -- set skin when enabled
        elseif option == "Disable" then
            self:SetEnabled(false)
            self:SetSkin(0) -- optional: reset skin when disabled
        elseif option == "Set Frequency: 96.2" then
            self:UpdateFrequency("96.2")
        elseif option == "Set Frequency: 96.3" then
            self:UpdateFrequency("96.3")
        end
    end
...
function ENT:GetEntityMenu(client)
    if not IsValid(client) or client:GetPos():DistToSqr(self:GetPos()) > (75 * 75) or not client:GetCharacter() then
        return
    end
    ...
    options["Set Frequency: 96.2"] = true
    options["Set Frequency: 96.3"] = true
