...
local SKIN = {}
derma.DefineSkin("ixdreams", "The dev skin for IXDREAMS.", SKIN)
...
function SKIN:PaintInfoBar(panel, width, height, color)
	...
	local newHeight = height * 0.8
	-- bar
	surface.SetDrawColor(color.r, color.g, color.b, 250)
	surface.DrawRect(5, (height - newHeight), width, newHeight)
...
function SKIN:PaintInfoBarBackground(panel, width, height)
	...
	surface.DrawRect(widthOffset, 0, width - widthOffset, height)
	surface.DrawOutlinedRect(widthOffset, 0, width - widthOffset, height)
...
function SKIN:PaintWorldHint(panel, w, h, font, background)
	if background then
	...
	surface.SetTextColor(panel.fontColor)
