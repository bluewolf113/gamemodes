...
function PANEL:Init()
	...
	infoLabel:SetText("Electric Dreams")
	...
	if (logo and !logo:IsError()) then
		...
		logoImage:SetSize(564, 201)
		logoImage:SetPos(halfWidth - logoImage:GetWide() * 0.5, halfPadding - 50)
		...
		-- local titleLabel = logoPanel:Add("DImage")
	...
	self.mainButtonList:DockMargin( ScrW()*0.32, ScrH()*0.435, 0, 0 )
end
...
function PANEL:Paint(width, height)
	...
	if (!ix.option.Get("cheapBlur", false)) then
		...
		surface.DrawTexturedRect(0, 0, width, height)
		ix.util.DrawBlur(self, Lerp((self.currentAlpha - 200) / 300, 0, 10))
	end
end
