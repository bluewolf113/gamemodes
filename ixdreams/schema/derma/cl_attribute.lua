...
function PANEL:SetText(text)
	...
	if self:GetIcon() then 
		self.bar.icon = self.bar:Add("DImage")
		self.bar.icon:Dock(LEFT)
		self.bar.icon:NoClipping(true)
		self.bar.icon:DockMargin(3,4,4,4)

		self.bar.icon:SetSize(15, 15)
		
		self.bar.icon:SetImage(self:GetIcon()) 
	end
end
