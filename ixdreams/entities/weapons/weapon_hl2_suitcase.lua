...
if CLIENT then
	...
	function SWEP:DrawWorldModel()
		...
		if (IsValid(_Owner)) then
			...
			local offsetVec = Vector(4, -1.1, 0)
...
function SWEP:GetViewModelPosition(pos, ang)
    -- Offset values (tweak these!)
    local offset = Vector(0, 0, 0) -- forward, right, up
    local rotate = Angle(0, 0, 0)    -- pitch, yaw, roll

    -- Apply rotation
    ang:RotateAroundAxis(ang:Right(), rotate.pitch)
    ang:RotateAroundAxis(ang:Up(), rotate.yaw)
    ang:RotateAroundAxis(ang:Forward(), rotate.roll)

    -- Apply position offset
    pos = pos + offset.x * ang:Forward()
    pos = pos + offset.y * ang:Right()
