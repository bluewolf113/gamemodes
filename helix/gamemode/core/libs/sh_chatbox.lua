...
do
	...
	hook.Add("InitializedConfig", "ixChatTypes", function()
		...
		-- Whisper chat.
		ix.chat.Register("w", {
			format = function(speaker, text, listener)
				local range = ix.config.Get("chatRange", 280)
				local distance = speaker:GetPos():Distance(listener:GetPos())
		
				if distance <= range * 0.25 then
					-- Close listeners see the actual message.
					return string.format("%s whispers, \"%s\"", speaker:GetName(), text)
				else
					-- Far listeners see "something."
					return string.format("%s whispers something", speaker:GetName())
				end
			end,
			GetColor = function(self, speaker, text)
			...
			CanHear = ix.config.Get("chatRange", 280),  -- Keep CanHear focused on range-checking.
			prefix = {"/W", "/Whisper"},
			description = "@cmdW",
