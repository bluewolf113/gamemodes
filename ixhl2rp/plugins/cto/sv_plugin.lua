...
if SERVER then
    util.AddNetworkString("ixDisableCamera")
    util.AddNetworkString("ixEnableCamera")
    
    net.Receive("ixDisableCamera", function(len, client)
        local camera = net.ReadEntity()
        if (not IsValid(camera)) or camera:GetClass() ~= "npc_combine_camera" then
            client:Notify("There is no Combine camera with that ID!")
            return
        end

        -- To disable, the camera must be in the "idle" state.
        if (camera:GetSequenceName(camera:GetSequence()) ~= "idlealert") then
            client:Notify("That camera is already disabled!")
            return
        end

        Schema:AddCombineDisplayMessage("Camera C-i" .. camera:EntIndex() .. " offline.")
        camera:Fire("Disable")
    end)
    
    net.Receive("ixEnableCamera", function(len, client)
        local camera = net.ReadEntity()
        if (not IsValid(camera)) or camera:GetClass() ~= "npc_combine_camera" then
            client:Notify("There is no Combine camera with that ID!")
            return
        ...
        -- To enable, the camera must be in the "idlealert" state.
        if (camera:GetSequenceName(camera:GetSequence()) ~= "idle") then
            client:Notify("That camera is already enabled!")
            return
        end

		Schema:AddCombineDisplayMessage("Camera C-i" .. camera:EntIndex() .. " online.")
        camera:Fire("Enable")
    end)
end