...
ITEM.isPoisoned = false  -- Defaults to not poisoned
...
function ITEM:SetVolume(vol)
    ...
    elseif vol == 0 then
        if self.emptyContainer then
            ...
            self:SetLiquid(nil)
            self.isPoisoned = false  -- Reset poison status when empty
        end
    else
...
ITEM.functions.ADrink = { -- consumes everything if below the max drinking volume, or the max drinking volume if greater
    ...
    OnRun = function(item)
        ...
        if(vol) then
            ...
            if item.isPoisoned then
                client:TakeDamage(math.random(10, 25))  -- Random damage between 10 and 25
                client:ChatPrint("You feel sick after drinking this...")
            end
        end
