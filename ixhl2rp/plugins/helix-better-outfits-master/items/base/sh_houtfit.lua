...
-- Maximum cleanliness value
ITEM.maxCleanliness = 100
...
if CLIENT then
    ...
    local function GetCleanlinessDescription(c)
        if c == 0        then return "Pristine"
        elseif c <= 20   then return "Good shape"
        elseif c <= 50   then return "Dusty"
        elseif c <= 75   then return "Dirty"
        else                  return "Filthy"
        end
    end

    function ITEM:PopulateTooltip(tooltip)
        local c = self:GetData("cleanliness", 0)
        local row = tooltip:AddRow("cleanliness")
        row:SetText(GetCleanlinessDescription(c))
        row:SetBackgroundColor(Color(255, 215, 0))
        row:SizeToContents()
    end
end

if SERVER then
    if not timer.Exists("UniversalClothingDirty") then
        timer.Create("UniversalClothingDirty", 5, 0, function()
            for _, ply in ipairs(player.GetAll()) do
                ...
                for _, item in pairs(char:GetInventory():GetItems()) do
                    if not item.bClothing then continue end
                    ...
                    if nxt >= item.maxCleanliness then
                        ply:Notify("Your outfit is now fully dirty!")
                    end
                end
            end
        end)
...
function ITEM:RemoveOutfit(client)
    ...
    -- Stop cleanliness tracking when unequipped.
    timer.Remove("CleanlinessTimer_" .. client:SteamID())
...
-- Makes an attachment depend on this outfit.
...
ITEM.functions.EquipUn = { -- Unequip function
    name = "Unequip",
    tip = "equipTip",
...
ITEM.functions.Equip = {
    ...
    OnRun = function(item)
        ...
        -- Cleanliness is not reset upon (re)equipping; it persists.
        ...
        item:OnEquipped()
        -- Start cleanliness tracking (fast interval for testing)
        item:StartCleanlinessTracking(client)
        return false
    end,
    OnCanRun = function(item)
        local client = item.player
        ...
        return !IsValid(item.entity) and IsValid(client) and item:GetData("equip") != true and 
            item:CanEquipOutfit() and hook.Run("CanPlayerEquipItem", client, item) != false and 
            item.invID == client:GetCharacter():GetInventory():GetID()
    end
}
