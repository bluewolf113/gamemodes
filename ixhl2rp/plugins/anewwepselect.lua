PLUGIN.name = "Weapon Select Redux"
PLUGIN.author = "Chessnut, Blue, Copilot"
...
if (CLIENT) then
    ...
    function PLUGIN:OnIndexChanged(weapon)
        ...
        if (IsValid(weapon)) then
            ...
            if (instructions and instructions:find("%S")) then
    ...
    -- Modified PlayerBindPress: when left ALT is held, do not intercept input.
    function PLUGIN:PlayerBindPress(client, bind, pressed)
        -- IMPORTANT: if left alt is being held, let all input pass by.
        if input.IsKeyDown(KEY_LALT) then return end

        bind = bind:lower()

        if (not pressed or (not bind:find("invprev") and not bind:find("invnext")
        and not bind:find("slot") and not bind:find("attack"))) then
            return
        end
